...
export default function Notificaciones({ navigation }) {
  ...
  useEffect(() => {
    if (sintomas) {
      ...
      // var fecha = "04/06/2022";
      let sintomasExisten = sintomas[fecha]?.síntomas;
      if (sintomasExisten) {
  ...
  function ThreeConcentrationsUnhealthy() {
    let concentraciones = CertainLevelConcentration(AQI_LEVEL_ELEMENT.unhealthy_sensgrp);
  ...
  function CertainLevelConcentration(level) {
    ...
    if (concentraciones) {
      if (aqiLevelCO(co) == level)
        unhealthyConcentrations.push("co");
      if (aqiLevelNO2(no2) == level)
        unhealthyConcentrations.push("no2");
      if (aqiLevelPM10(pm10) == level)
        unhealthyConcentrations.push("pm10");
      if (aqiLevelO3(o3) == level)
        unhealthyConcentrations.push("o3");
      if (aqiLevelSO2(so2) == level)
        unhealthyConcentrations.push("so2");
    }
          ...
          >
            ...
            <ScrollView style={{ height: "100%" }}>
              ...
              {CertainLevelConcentration(AQI_LEVEL.unhealthy).length > 0 && (
                <Alerta
                  fontSize={15}
                  titleText={`Por las concentraciones de ${CertainLevelConcentration(AQI_LEVEL.unhealthy).join(", ")}`}
                  suggestionText={"No salgas sin mascarilla \n\u2022 Cierra ventana \n\u2022 No hagas ejercicio"}
                  background="#DE2F24"
                  height={120}
                />
              )}
              ...
              {CertainLevelConcentration(AQI_LEVEL.hazardous).length > 0 && (
                <Alerta
                  fontSize={15}
                  titleText={`Por las concentraciones de ${CertainLevelConcentration(AQI_LEVEL.hazardous).join(", ")}`}
...
const Alerta = ({ titleText, suggestionText, background, fontSize, height }) => {
  return (
    <View
      style={[
